...
class PengRobinson:
    def __init__(self, compound, T_c, P_c, omega, T, verbose=False):
        ...
        self.omega = omega
        self.T = T
        self.R = 8.314  # Constante de gases ideales en J/(mol·K)
        
        # Parámetros de Peng-Robinson
        self.a = 0.45724 * (self.R**2 * self.T_c**2) / self.P_c * \
                 (1 + (0.37464 + 1.54226 * self.omega - 0.26992 * self.omega**2) * \
                  (1 - np.sqrt(self.T_c / T)))**2
        self.b = 0.07780 * self.R * self.T_c / self.P_c

        # Calcular el K_value inicial usando T y P críticos
        self.K_value = self.calculate_K(T, self.P_c)
        
        if verbose:
            print(f"Parametros de Peng-Robinson para {self.compound}:")
            print(f"     a = {self.a:.2f}  b = {self.b:.2f}")
            print(f"     K value at {T} K and {self.P_c} Pa: {self.K_value}")
